...
def _build_cover_patterns():
    ...
    return [
        ...
        (re.compile(
            r'^[\s\d\.\-\*]*(שומת\s+נכס\s+מקרקעין\s+\S+\s*-\s*)'
            r'(' + _VAL_TO_END + r')'
            r'(?=\s*$)'
        ), ["סוג שומה"]),
        ...
        (re.compile(r'^[\s\d\.\-\*]*(רחוב\s*:?\s*)(' + _VAL_TO_END + r')(?=\s*$|,)'), ["רחוב"]),

        # 7. עיר: VALUE
        (re.compile(r'^[\s\d\.\-\*]*(עיר\s*:\s*)(' + _VAL_TO_END + r')(?=\s*$)'), ["עיר"]),

        # 8. מזמין השומה: VALUE
        (re.compile(r'^[\s\d\.\-\*]*(מזמין\s+השומה\s*:\s*)(' + _VAL_TO_END + r')(?=\s*$)'), ["מזמין השומה"]),

        # 9. מספר תיק: VALUE (Often appears anywhere, but let's anchor it to be safe)
        (re.compile(r'^[\s\d\.\-\*]*(מספר\s+תיק\s*:\s*)(' + _VAL + r')'), ["מספר תיק"]),
    ]
...
def _build_section6_patterns():
    ...
    return [
        ...
        (re.compile(r'^[\s\d\.\-\*]*(שטח\s+(?:ה)?חלקה\s*:?\s*)(' + _VAL_TO_EOL + r')(\s*\.?)'), ["שטח חלקה"]),

        # 11. שטח בנוי: VALUE.
        (re.compile(r'^[\s\d\.\-\*]*(שטח\s+בנוי\s*:?\s*)(' + _VAL_TO_EOL + r')(\s*\.?)'), ["שטח בנוי"]),

        # 12. תיאור זכויות: VALUE.
        (re.compile(r'^[\s\d\.\-\*]*(תיאור\s+זכויות\s*:?\s*)(' + _VAL_TO_EOL + r')(\s*\.?)'), ["תיאור זכויות"]),

        # 13. החלקה הנישום: VALUE. or החלק הנישום: VALUE.
        (re.compile(r'^[\s\d\.\-\*]*(החלק(?:ה)?\s+הנישום\s*:?\s*)(' + _VAL_TO_EOL + r')(\s*\.?)'), ["החלקה הנישום"]),

        # 14. מיקום: רחוב VALUE, VALUE. → רחוב + עיר
        # City value: greedy match up to trailing period
        (re.compile(
            r'^[\s\d\.\-\*]*(מיקום\s*:\s*רחוב\s+)'
            r'(' + _VAL_TO_END + r')'
            r'(\s*,\s*)'
            r'(' + _VAL_TO_EOL + r')'
            r'(\.?)'
