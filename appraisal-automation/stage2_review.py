...
★ כלל-על: אל תדווח על שדות תקינים. אל תדווח על שדה כחסר אם המידע קיים
  במסמך בכל צורה שהיא — גם אם הוא בסעיף אחר, בטבלה, או תחת תווית שונה.
  חפש היטב בכל המסמך לפני שתדווח על חסר.

בדוק אך ורק:
1. עקביות לוגית (category: "logic"):
   - השווה שטחים/ערכים/גוש-חלקה בין הכותרת, סעיף פרטי הנכס, והחלק המפורט. סתירה = דווח.
   - בדוק טעויות אריתמטיות בטבלאות.
2. חסרים — שדות חובה (category: "missing", severity: "high"):
   - דווח רק אם חיפשת בכל המסמך ולא מצאת את המידע בשום צורה.
   - שדות חובה: תב"ע (תכנית מתאר), תאריך סיור, נסח טאבו, הצהרת שמאי, תנאים מגבילים, סעיפים 14-15.
   - אל תדווח על שדות שנמצאים בטבלה/סעיף אחר. אם הנתון קיים — הוא לא חסר.
...
כלל אינדקס: השתמש רק במספרים מטבלת האינדקס. אל תדווח על סעיפי הגבלת אחריות.\
"""
